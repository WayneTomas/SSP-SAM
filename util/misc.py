...
def collate_fn_with_mask(batch):
    ...
    img = torch.stack(batch[0])     # transformed后的图像本身
    mask = torch.stack(batch[1])    # 图像的transformer mask，与transformed后的图像等大的全false张量，表示所有token都可见
    samples = NestedTensor(img, mask)
    targets, task_id_list = tensor_dict_from_dict_list(batch[2])      # 包括phrase, ori_size, bbox, word_id, word_mask, etc.
    image_sam = torch.stack(batch[3])                   # 适合SAM输入的图像尺寸, 1024*1024
    img_mask = torch.cat(batch[4],dim=0)                # 处理过后的用于计算loss的gt mask，该mask经过了与SAM同样的预处理，然后差值到512，最终的尺寸是bs * 1 * 512 * 512
    return tuple([samples, targets, image_sam, img_mask, task_id_list])
...
def tensor_dict_from_dict_list(tensor_list: List[dict]):
    ...
    if 'task_id' in keys:
        keys.remove('task_id')
        task_id_list = [t['task_id'] for t in tensor_list]

    tensor_dict = dict()
    for k in keys:
        tensor_dict[k] = torch.stack([t[k] for t in tensor_list], dim=0)
    return TensorDict(tensor_dict), task_id_list
