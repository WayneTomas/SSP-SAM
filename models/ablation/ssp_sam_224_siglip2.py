...
class SSP_SAM(nn.Module):
    def __init__(self, clip_pretrained, args, device='cpu') -> None:
        ...
        self.patch_proj = nn.Linear(self.clip_embedding_dim, 768)
        self.sent_proj = nn.Linear(self.clip_embedding_dim, 768)
    ...
    def forward(self, image, image_mask, text, text_mask, sam_image, ori_size, target_dict):
        ...
        bs = image.shape[0]
        image_features = self.encoder.vision_model(image).last_hidden_state
        image_features = self.patch_proj(image_features)
        image_features = image_features / image_features.norm(dim=1, keepdim=True)

        txt_out = self.encoder.text_model(text)
        tmp_cls_embeddings = txt_out.pooler_output
        class_embeddings = self.sent_proj(tmp_cls_embeddings)
        word_token_embedding = txt_out.last_hidden_state
        ...
        similarity_map = get_similarity_map(similarity[:, :, :])    # 由于siglip这里拿出来的image_features已经是hidden state了，是bs*196*768，所以不需要similarity[:, 1:, :]
        ...
        select_image_features = image_features[:, :, :].view(-1, self.vit_patch_size, self.vit_patch_size, self.clip_embedding_dim) * similarity_map
        ...
        bbox_token = hs[0]
        multi_prompt = hs[1:self.query_token_num+1].permute(1, 0, 2)
