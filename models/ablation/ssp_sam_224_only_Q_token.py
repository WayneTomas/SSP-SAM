...
class SSP_SAM(nn.Module):
    ...
    def forward(self, image, image_mask, text, text_mask, sam_image, ori_size, target_dict):
        ...
        clip_text_embed = self.clip_proj(class_embeddings)
        ...
        select_image_features = image_features[:, 1:, :].view(-1, self.vit_patch_size, self.vit_patch_size, self.clip_embedding_dim)
        select_image_features = self.clip_img_proj(select_image_features)
        # y_2d = weighted_emb.squeeze().unsqueeze(-1).unsqueeze(-1)
        y_2d = clip_text_embed.squeeze().unsqueeze(-1).unsqueeze(-1)
        ...
        # ###########################################################################
        fuse_img_feat = select_image_features.view(bs, self.vit_patch_size * self.vit_patch_size, 512)
        ...
        bbox_token = hs[0]
        multi_prompt = hs[1:(self.query_token_num+1)]
