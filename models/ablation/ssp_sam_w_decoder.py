...
class SSP_SAM(nn.Module):
    def __init__(self, clip_pretrained, args, device='cpu') -> None:
        ...
        self.d_model = 256
        self.vl_transformer = nn.TransformerDecoder(
    ...
    def forward(self, image, image_mask, text, text_mask, sam_image, ori_size, target_dict):
       ...
       # multi-modal fusion特征
        ...
        x_multi_modal = self.mlp(x_multi_modal)  # (N, B, C)

        # query tokens
        bbox_src = self.reg_token.weight.unsqueeze(1).repeat(1, bs, 1)
        tgt_src = self.prompt_token.weight.unsqueeze(1).repeat(1, bs, 1)
        vl_query = torch.cat([bbox_src, tgt_src], dim=0)  # (1+L) x B x C

        # masks
        tgt_mask = None  # 不需要 sequence mask
        tgt_key_padding_mask = torch.zeros(bs, vl_query.size(0), dtype=torch.bool, device=vl_query.device)
        memory_key_padding_mask = torch.zeros(bs, x_multi_modal.size(0), dtype=torch.bool, device=x_multi_modal.device)

        # DETR decoder forward
        output = self.vl_transformer(
            tgt=vl_query.permute(1, 0, 2),       # B x (1+L) x C -> TransformerDecoder 默认 batch_first=True
            memory=x_multi_modal.permute(1, 0, 2), # B x N x C
            tgt_key_padding_mask=tgt_key_padding_mask,
            memory_key_padding_mask=memory_key_padding_mask
        )  # 输出 B x (1+L) x C

        # transpose回原来的维度
        hs = output.permute(1, 0, 2)  # (1+L) x B x C
