...
class SSP_SAM(nn.Module):
    def __init__(self, clip_pretrained, args, device='cpu') -> None:
        ...
        image_size = 336
        self.vit_patch_size = int(image_size / 14)
    ...
    def forward(self, image, image_mask, text, text_mask, sam_image, ori_size, target_dict):
        ...
        out['x_multi_modal'] = x_multi_modal.detach().cpu().numpy()
        out['fuse_img_feat'] = fuse_img_feat.permute(1,0,2).detach().cpu().numpy()
        out['verify_score'] = verify_score.permute(1,0,2).detach().cpu().numpy()
        if self.is_pretrain:
            return out
        ...
        for i in range(bs):
            ...
            # sam_type_gt_coord.append(self.apply_boxes_torch(target_dict['orig_bbox'][i], ori_size[i].detach().cpu().tolist()))
        sam_bbox = torch.cat(sam_type_pseudo_coord,dim=0)
        # sam_type_gt_bbox = torch.cat(sam_type_gt_coord,dim=0)
        ...
        out['multi_prompt'] = multi_prompt.detach().cpu().numpy()
        out['upscaled_embedding'] = upscaled_embedding.detach().cpu().numpy()
        # out['pred_boxes'] = sam_bbox.unsqueeze(0)
        # out['gt_boxes'] = sam_type_gt_bbox.unsqueeze(0)
