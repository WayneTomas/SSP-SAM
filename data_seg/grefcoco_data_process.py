...
"""
# import debugpy;debugpy.connect(('22.8.41.8', 6792))
...
class G_REFER:
    def __init__(self, data_root, dataset='grefcoco', splitBy='unc'):
        ...
        if dataset in ['grefcoco']:
            ...
            # write by wayne
            # 针对我们自己的情况的修改
            self.IMAGE_DIR = "data/other/coco/train2014"
        else:
    ...
    def save_mask(self, mask):
        binary_image = Image.fromarray(mask_info['mask'] * 255)
        binary_image.save('binary_image.png')
    
    def visulize_bbox(self, bbox):
        image = Image.open("COCO_train2014_000000000072.jpg").convert("RGB")
        ...
        output_path = 'COCO_train2014_000000000072_with_bbox.png'
        image.save(output_path)
    
    def visual_merged_bbox(self, bbox1, bbox2):
        image = Image.open("COCO_train2014_000000000072.jpg").convert("RGB")
        draw = ImageDraw.Draw(image)
        # 计算右下角的坐标
        ...
        draw.rectangle(merged_bbox, outline='red', width=2)
        output_path = 'COCO_train2014_000000000072_with_merged_bbox.png'
        image.save(output_path)


def bbox_process(bboxs):
    converted_bboxes = []
    for bbox in bboxs:
        x, y, width, height = bbox
        x2 = x + width
        y2 = y + height
        converted_bboxes.append([x, y, x2, y2])
    
    # 一次性选出合并后的边界框
    x1 = min([bbox[0] for bbox in converted_bboxes])
    y1 = min([bbox[1] for bbox in converted_bboxes])
    x2 = max([bbox[2] for bbox in converted_bboxes])
    y2 = max([bbox[3] for bbox in converted_bboxes])
    merged_bbox = [x1, y1, x2, y2]
...
def prepare_dataset(dataset,splits,output_dir,generate_mask=False):
    ...
    for split in splits:
        ...
        for i in ref_ids:
            ...
            bboxs = bbox_process(bboxs)    # 针对gref，合并多个bbox，转为x1y1x2y2格式
            ...
            if generate_mask:
                np.save(os.path.join(output_dir,'masks',dataset,str(i)+'.npy'),refer.getMaskByRef(ref=refs, merge=True)['mask'])  #if need seg mask ,set it!
            split_num+=1
...
data_root = "/cpfs01/user/tangwei/codes/ssp_sam_submit_tip/ssp_sam_final/data_seg"
dataset = "grefcoco"
split = "unc"
# splits = ['train', 'val', 'testA', 'testB']
refer = G_REFER(data_root=data_root, dataset=dataset, splitBy=split)

print ('dataset [%s_%s] contains: ' % (dataset, split))
...
if __name__ == "__main__":
    ...
    prepare_dataset("grefcoco", splits, output_dir='data_seg', generate_mask=True)