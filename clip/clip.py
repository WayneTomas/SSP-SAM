...
def clip_feature_surgery(image_features, text_features, redundant_feats=None):
    ...
    if redundant_feats != None:
        ...
        prob = image_features[:, :1, :] @ text_features.permute(0,2,1)
...
def similarity_map_to_points(sm, shape, t=0.8, down_sample=2):
    ...
    down_side = torch.div(side, down_sample, rounding_mode='trunc')
    ...
    num = min((sm >= t).sum(), torch.div(sm.shape[0], 2, rounding_mode='trunc'))
    ...
    for idx in rank[-num:]:
        ...
        y = min((torch.div(idx, w, rounding_mode='trunc') + 0.5) * scale_h, shape[0] - 1)
