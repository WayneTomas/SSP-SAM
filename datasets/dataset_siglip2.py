...
from .utils import read_examples, convert_examples_to_features_siglip
...
siglip_model_name = "./siglip2"
model = AutoModel.from_pretrained(siglip_model_name)
img_processor = AutoProcessor.from_pretrained(siglip_model_name)
from transformers import AutoTokenizer

tokenize = AutoTokenizer.from_pretrained(siglip_model_name)
...
class VGDataset(Dataset):
    ...
    def getitem__PIL(self, idx):
        ...
        try:
            features = convert_examples_to_features_siglip(
                examples=examples, tokenizer=self.tokenizer, prompt_templates=['{}.'], model=None, cat_name=None)
        except:
                print(target['phrase'])
